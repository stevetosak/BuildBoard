...
const formatTime = (time: string) => {
	...
	return (
		<>
			...
			<span>{`${date.getMinutes()}:${date.getHours()}`}</span>
		</>
	);
};
...
const ThreadsComponent = () => {
	const { data, isPending } = useQuery({
		queryKey: ["init"],
		queryFn: handleInitFetch,
	});

	return (
		<section className="grid grid-rows-[8em_auto] h-screen overflow-scroll" style={{ scrollbarWidth: "none" }}>
			<SearchBar className="w-1/2 justify-self-center self-center"/>
			<div
				className="flex flex-col gap-5 p-5 row-start-2 items-center"
			>
				{isPending
							...
							>
								<CardHeader className="w-full">
									<div className="w-full grid-cols-[repeat(2,max-content)_auto] gap-2 items-center grid">
										...
										<span className="text-sm font-medium text-gray-400 hover:text-white justify-self-end">
											{formatTime(thread.meta.uploadedAt)}
										</span>
									</div>
								...
								<CardContent className="text-left ">
									<p className="text-sm text-gray-300 truncate">
										{thread.content.shortDescription}
									</p>
								</CardContent>
								<CardFooter className="flex wrap gap-5">
									{thread.content.tags.map((tag) => (
										<div className="bg-[#2b6c4f] hover:bg-[#3a976d] rounded-xl px-2 p-1">
											<span>#{tag}</span>
										</div>
									))}
								</CardFooter>
