...
public class NamedThreadsController {
    ...
    public NamedThreadsController(SearchService searchService, TagService tagService, BBUserDetailsService bbUserDetailsService, SecurityContextRepository securityContextRepository) {
    ...
    public ResponseEntity<Page<NamedThreadDTO>> search(
        ...
        Page<NamedThread> namedThreads = searchService.search(query, filters, type, PageRequest.of(page, 10));
        NamedThreadDTO.NamedThreadDTOBuilder builder = NamedThreadDTO.builder();
        return ResponseEntity.ok(
                namedThreads.map(namedThread ->
                                builder.createdAt(namedThread.getCreatedAt())
                                        .creator(new NamedThreadDTO.Creator(namedThread.getUser().getUsername(), namedThread.getUser().getAvatarUrl()))
                                        .content(new NamedThreadDTO.Content(
                                                namedThread.getTitle(),
                                                namedThread.getContent(),
                                                namedThread.getTags().stream().map(Tag::getName).toList()
                                        )).build()
                                         )
                                );
    }
